...
import {
  ...
  updateTransaction,
  removeTransaction,
} from '../../redux-toolkit/transactionSlice.js'
import { showSuccessToast, showErrorToast } from '../../untils/Toaste.js'
import { v4 as uuidv4 } from 'uuid'
import { transactionTypes } from '../TransactionItem.js'
import { toggleModal, resetTransactionData } from '../../redux-toolkit/modalSlice'

const ModalExpense = () => {
  const dispatch = useDispatch()
  const { isShow, title, transactionData } = useSelector((state) => state.modal)
  ...
  return (
    ...
    >
      <div className='px-4 py-6 space-y-4 text-[14px] font-bold xl:w-[70vw] md:mx-auto'>
        <div className='flex border rounded-[15px] h-[32px] md:h-[60px] md:rounded-full bg-[#D9D9D9]'>
          <button
            className={`flex-1 h-full rounded-l-full md:h-[60px] md:text-2xl ${
              isExpense ? 'bg-[#EF8767] text-white' : 'bg-transparent text-black'
            }`}
        ...
        <div>
          <label className='text-sm font-semibold md-text-xl'>Expense Receipt Image</label>
          <Upload
            beforeUpload={(file) => {
              setReceipt(file)
              return false
        ...
        <div className='flex justify-between mt-4'>
          {transactionData ? (
            ...
            <Button
              onClick={() => {
                dispatch(toggleModal(false)) // Đóng modal
                dispatch(resetTransactionData()) // Reset transactionData
              }}

...
const ExpenseItem = ({ name, description, amount, onClick }) => {
  const transactionType = transactionTypes.find((type) => type.type === name)
  const icon = transactionType ? transactionType.icon : null

  return (
    <div className='flex items-center  p-2 rounded-lg cursor-pointer ' onClick={onClick}>
      <div
      ...
      <div className='ml-3'>
        ...
        <p className='text-[12px] md:text-xl font-semibold text-[#AEABAB]'>{description}</p>
      </div>
